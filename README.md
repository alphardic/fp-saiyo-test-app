...
* Project URL
* anon public key
* service\_role key(絶対に外部に漏らさないこと)
...
```bash
node -r dotenv/config supabase/seed.js dotenv\_config\_path=.env.local
```
...
```sql
insert into public.admins (user\_id, name)
...
4. 環境変数(`.env.local` と同じ内容)をプロジェクトの Settings > Variables and secrets に設定
   ...
   * `SUPABASE\_SERVICE\_ROLE\_KEY` は必ず Secret(暗号化)として設定し、リポジトリには絶対にコミットしないこと
5. デプロイ完了後、候補者向けリンクは `https://<プロジェクト>.workers.dev/exam/<candidatesテーブルのinvite\_token>` になります
